...
def proxyget(url): 
	try:
		...
		print("\nПроизошла ошибка, переход к следующему веб-сайту.")
...
def main(): 
	try:
		...
		print ("\nВыполняется загрузка с socks-proxy.net...")
		...
		for position, url in enumerate(nurls):
			proxyget (url)
			print("Завершенные загрузки: (%s/%s)\nТекущий IP в списке прокси: %s" % (position+1, len(nurls), len(open("proxy.txt").readlines())))
...
def start_tawkunsocks():
	while True:
		choice = input("\nХотите скачать socks? [Y/n] > ")
		if choice == 'Y' or choice == 'y' or choice == 'yes' or choice == 'Yes' or choice == '': 
			main()
			break
		if choice == 'N' or choice == 'n' or choice == 'no' or choice == 'No': 
