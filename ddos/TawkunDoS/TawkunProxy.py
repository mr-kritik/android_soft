...
def proxyget(url): 
	try:
		...
		print("Произошла ошибка, перейдите к следующему веб-сайту.\n")
...
def proxyget2(url): 
	try:
		...
		print("Произошла ошибка, перехожу к следующему веб-сайту.\n")
...
def blogspotget(url, word, word2): 
	try:
		...
		print("произошла ошибка, перейдите на следующий сайт.\n")
...
def proxylist(): 
	...
	print("Текущий IP в списке прокси: %s" % (len(open("proxy.txt").readlines())))
	print ("\nProxylist Updated!\n")
...
def main(): 
	try:
		...
		print ("\nВыполняется загрузка из списка бесплатных прокси...")
		...
		print ("\nВыполняется загрузка с нескольких зеркал...")
		for position, url in enumerate(nurls):
			proxyget(url)
			print("Завершенные загрузки: (%s/%s)\nCurrent IP в проксилисте: %s" % (position+1, len(nurls), len(open("proxy.txt").readlines())))

		print ("\nВыполняется загрузка с foxtools...")
		...
		while True:
			...
			if choice == 'N' or choice == 'n' or choice == 'no' or choice == 'No':
				...
				print ("Пожалуйста, пишите правильно.")
...
def start_tawkunproxi():
	while True:
		choice = input("\nВы уверены, что хотите скачать прокси? [Y/n] > ")
		if choice == 'Y' or choice == 'y' or choice == 'yes' or choice == 'Yes' or choice == '':
			main()
			break
		if choice == 'N' or choice == 'n' or choice == 'no' or choice == 'No':
